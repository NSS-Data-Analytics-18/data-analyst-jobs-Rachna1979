...
FROM data_analytics_jobs
WHERE location = 'TN'; //21

SELECT COUNT(location)
FROM data_analytics_jobs
WHERE location = 'TN' OR location = 'KY'; //27

---------4.	How many postings in Tennessee have a star rating above 4?-----------
SELECT COUNT(title)
FROM data_analytics_jobs
WHERE location = 'TN'AND star_rating > 4;  //3
...
GROUP BY location
ORDER BY avg_rating DESC;  //NE: Nebraska 4.199999

--------7.	Select unique job titles from the data_analyst_jobs table. How many are there?-----

SELECT COUNT(DISTINCT(title))
FROM data_analytics_jobs;  //881
...
FROM data_analytics_jobs
WHERE title ILIKE '%Analyst%';   //1669
